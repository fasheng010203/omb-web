...
public final class QRCodeUtil extends LuminanceSource {
    ...
    private static String filePathnew;
    ...
    public static String generateQRCode(String text,String  path)
            throws Exception {

        int width = 250; // 二维码图片的宽
        int height = 250; // 二维码图片的高
        String format = "png"; // 二维码图片的格式

        Date now = new Date();
        StringBuilder qrBuilder = new StringBuilder();

        qrBuilder.append(DateUtil.formateDate(now, DateUtil.DATEFORMAT_STR_YMDHMSSS));
        qrBuilder.append(ToolUtil.getRandom(4).toString());

        String fName=qrBuilder.toString()+"."+format;
        String pathName=path+fName;


        // 生成二维码图片，并返回图片路径
        String resultPathName = generateQRCode(text, width, height, format, pathName);
...
//        try {
//
//            //String s=generateQRCode("hellllllllll");
//           // System.out.println("解析出二维码的图片的内容为： " + s);
//
////            // 生成二维码图片，并返回图片路径
////            String pathName = generateQRCode(text, width, height, format, "/eric/tem/new.png");
